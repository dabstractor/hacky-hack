...
const workflowScenarios = [
  { name: 'initial run', input: { /* ... */ }, expectedChanges: 0 },
  { name: 'with single change', input: { /* ... */ }, expectedChanges: 1 },
  { name: 'with multiple changes', input: { /* ... */ }, expectedChanges: 5 }
]

test.concurrent.each(workflowScenarios)('workflow: $name', async ({ input, expectedChanges }) => {
...
describe('Complete E2E workflow: Multi-run with delta processing', () => {
  ...
  it('verifies state persistence across runs', async () => {
    ...
    expect(state.files.find((f: any) => f.path === 'file1.txt').hash).not.toBe(state.initialHashes.file1)
    expect(state.files.find((f: any) => f.path === 'file2.txt').hash).toBe(state.initialHashes.file2)
  })
